...
同样到道理适用于右移和除法，但是关于这两个运算符，还有更深层次的区别，参见整数那一章。
...
联系运算符那一章，建议对整数放在条件表达式里的情况加上`!= 0`，于是原本正确的
...
最直观的是运算结果的不等价。即使我们只关心结果的真假，也未必相同：例如`1 & 2 == 0`为假，但`1 && 2 == 1`为真。

更重要的是，`&&`和`||`作为逻辑运算符，有非常重要的“短路求值”的特性。我会在后文详细介绍。

`~`和`!`区别也很大，不应该混淆，这里就不再赘述了。
...
函数`f3()`可能在`f1()`和`f2()`之前、之后甚至之间调用——因为这就这个表达式的计算过程而言是不影响的——谁先被调用不影响我计算的是`f1() * f2() + f3()`。
...
这会引发未定义行为（undefined behavior）——需要注意的是，未定义行为未必导致未定义结果，例如`j = i++ + ++i`无论以哪种求值顺序进行计算，都会得到`j = i += 2`的结果，但是过程是我们无法预测的。当然，更多情况下，我们还是会遇到未定义的结果。

-----

然而，标准还是明确规定了四个运算符的求值顺序，它们是`&&`、`||`、`?:`和`,`。
...
注意最后一行不是多维数组的访问（这对有多维数组的语言使用者来说可能比较熟悉），逗号在这里只是一个普通的运算符而已。
...
等等，我们是不是有一种更直接的方法：（想一想，下面哪个括号是不能省去的？）

```C
cond() ? (then_clause(), 1) : (else_clause(), 0); 
```

没错，你可能才意识到，其实条件运算符也完全具备短路求值的特性——如果条件的真假值没有确定，后面两个子句也不会被执行。即时确定了，也只会选择其中一个执行，另外一个不会执行。
