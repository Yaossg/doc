...
最后一行可能令人略感疑惑，它实际上发生了如下的转换：

![](implicit-cycle.png)

## 布尔类型

布尔类型是 C99 才加入 C 的，但是这样的转换一直都存在，那就是当上下文需要一个布尔值时（例如`if`中，或`&&`运算符两边等），算术类型就会隐式转换为布尔值（真或假）来参加运算。

在学习 C 之初，我们就接触到一个概念：在整数转为布尔值的时候，零为假，非零为真，而反过来转换的时候呢，假为零，真为一，是不对称的。但是别忘了，算数类型不仅包括整型，还有浮点型。因此要注意下面这样的情况：

![](boolean-conv.png)

总的来说，加一个`!= 0`是解决问题最有效的方法。但还是那句话，别忘了加括号。

还有一点需要注意，隐式转为布尔类型，除了在分支和循环条件中之外，还有逻辑运算符`&&`、`||`两侧和`!`运算符后面，别把它们忘了。
...
实现定义的原因不难理解：毕竟有符号数怎么实现的也是实现定义的。好在我们通常不需要担心这个问题，如同整数那一章说的那样。
...
注意`sizeof(char) == 1`不需要假定，因为这是必须的。也要注意，上面的假定完全是为了叙述方便，并非四海皆准。
...
注意，这样的提升是无视符号性（signedness）的，也就是是说`unsigned char`和`unsigned short`经过提升之后还是会得到`int`（除非`short`和`int`一样大——这在 16 位的机器上比较常见——那么会被提升到`unsigned int`）。
...
开始之前我们先定义一下整数类型的阶，注意我们这里说的阶指的是它的地位而不是实际的`sizeof`大小。
...
首先，`'a'`是`int`的（忘了吗？看看本章开头），然后被转为`char`，接着被提升为`int`。因为两侧都是`signed`的，属于情况 #1，所以直接取高阶者，即将左侧的`int`转为`long`。最后的结果是`2L`。
...
问题就出在，两侧的共同类型是`unsigned int`，也就是说，左边的`-1`被转换为无符号数再与`i`比较，而这显然是假的！解决方法是，将`i`也转换为有符号数再进行比较。
