...
答案是`34`。八进制总是令人着迷。你还会再见到它的。

-----

八进制转义序列最多接受三位八进制数，且会在第一个非八进制数字处停止。十六进制转义序列没有这个限制，接受任意位十六进制数，但也会在第一个非十六进制数字处停止。（你也可以将其理解为一种贪心法）

如果转义序列的值超出了它字符类型的范围，结果是未定义的。
...
相信你看到三字符满脑子肯定都是`??!`。它早就已经过时，还带来这么多问题。好在 C++17 已经正义删除了这个特性。
